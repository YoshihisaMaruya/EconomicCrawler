...
inifile.read(config_path)

# http://hello-apis.blogspot.jp/2013/03/twitterapi.html
CK = str(unicode(inifile.get('key', 'CK')))  # Consumer Key
CS = str(unicode(inifile.get('key', 'CS'))) # Consumer Secret
AT = str(unicode(inifile.get('key', 'AT')))  # Access Token
AS = str(unicode(inifile.get('key', 'AS')))  # Accesss Token Secert
print("keys are : CK => " + CK + ", CS => " + CS + ", AT => " + AT + ", AS => " + AS)
...
if __name__ == '__main__':
    for word in ast.literal_eval(unicode(inifile.get('twit', 'words'), 'UTF-8')):
        print("get : " + word)
        output_file = output_path + "/" + word + "_" + today + ".csv"
        print("output to : " + output_file)
        getter = TweetsGetter.bySearch(word)
        f = open(output_file, 'w') # 書き込みモードで開く
        cnt = 0
	print(datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
        for tweet in getter.collect():
	    cnt += 1
            f.write('\"{}\",\"{}\",\"{}\",\"{}\"\n'.format(tweet['id'], tweet['created_at'], '@'+tweet['user']['screen_name'], tweet['text'].encode('utf-8').replace('\"', '') .replace('\n', '')))
        f.close()